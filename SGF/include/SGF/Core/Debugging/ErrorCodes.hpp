...
namespace SGF {
    namespace GPU {
		...
		SGF_ERROR(1422, QUEUE_WAIT_IDLE, "failed to wait for queue");
		...
		SGF_ERROR(1105, CREATE_GRAPHICS_PIPELINE, "failed to create graphics pipeline");
		...
		SGF_ERROR(1108, RESET_COMMAND_POOL, "failed to reset command pool");
		...
		SGF_ERROR(1110, CREATE_QUERY_POOL, "failed to create semaphore");
		...
		SGF_ERROR(1005, LOGICAL_DEVICE_NOT_CREATED, "failed to create logical device");

		SGF_ERROR(1004, PHYSICAL_DEVICE_NOT_FOUND, "failed to find suitable graphics device");
		SGF_ERROR(1004, PHYSICAL_DEVICE_INVALID, "invalid physical device");
		SGF_ERROR(1004, PHYSICAL_DEVICE_NOT_SUITED, "physical device does not meet requirements");
		SGF_ERROR(1004, PHYSICAL_DEVICE_ENUMERATION, "failed to enumerate graphics devices");
