...
#define SGF_DEFINE_NON_DISPATCHABLE_HANDLE(name) struct name##_T; typedef name##_T* name;
	...
	struct DescriptorSetBinding {
		...
		inline DescriptorSetBinding(uint32_t binding, DescriptorType type, uint32_t descriptorCount, Flags<ShaderStage> stages, const Sampler* pImmutableSamplers = nullptr) 
			: binding(binding), descriptorType(type), descriptorCount(descriptorCount), stageFlags(stages), pImmutableSamplers(pImmutableSamplers) {}
		inline DescriptorSetBinding() {}
	};
