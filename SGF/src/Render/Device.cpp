...
#pragma region PHYSICAL_DEVICE_HELPER 
    ...
    bool checkPhysicalDeviceSurfaceSupport(VkPhysicalDevice physicalDevice) {
        ...
        for (uint32_t i = 0; i < queueFamilyCount; ++i) {
            if (glfwGetPhysicalDevicePresentationSupport(g_VulkanInstance, physicalDevice, i) == GLFW_TRUE) {
                return true;
            }
        }
        return false;
    ...
    bool checkPhysicalDeviceQueueSupport(VkPhysicalDevice device, uint32_t graphicsCount, uint32_t computeCount, uint32_t transferCount) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            if (!hasPresent) {
                hasPresent = glfwGetPhysicalDevicePresentationSupport(g_VulkanInstance, device, i) == GLFW_TRUE;
            }
    ...
    void Device::getQueueCreateInfos(VkPhysicalDevice device, uint32_t* pIndexCount, VkDeviceQueueCreateInfo* pQueueCreateInfos, float* pQueuePriorityBuffer) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            if (!hasPresent && glfwGetPhysicalDevicePresentationSupport(g_VulkanInstance, device, i)) {
                this_present = true;
                hasPresent = true;
            }
...
#pragma endregion HELPER_FUNCTIONS
    void Device::pickPhysicalDevice(const DeviceRequirements& r) {
        uint32_t count;
        vkEnumeratePhysicalDevices(SGF::g_VulkanInstance, &count, nullptr);
        std::vector<VkPhysicalDevice> physical_devices(count);
        vkEnumeratePhysicalDevices(SGF::g_VulkanInstance, &count, physical_devices.data());
    ...
    void Device::Terminate() {
        ...
        vkDestroyDevice(logical, SGF::g_VulkanAllocator);
...
#pragma region DESTRUCTORS
    void Device::Destroy(VkFence fence) const {
        assert(fence != VK_NULL_HANDLE);
        vkDestroyFence(logical, fence, SGF::g_VulkanAllocator);
        TRACK_FENCE(-1);
    }
    void Device::Destroy(VkSemaphore semaphore) const {
        assert(semaphore != VK_NULL_HANDLE);
        vkDestroySemaphore(logical, semaphore, SGF::g_VulkanAllocator);
        TRACK_SEMAPHORE(-1);
    }
    void Device::Destroy(VkBuffer buffer) const {
        assert(buffer != VK_NULL_HANDLE);
        vkDestroyBuffer(logical, buffer, SGF::g_VulkanAllocator);
        TRACK_BUFFER(-1);
    }
    void Device::Destroy(VkImage image) const {
        assert(image != VK_NULL_HANDLE);
        vkDestroyImage(logical, image, SGF::g_VulkanAllocator);
        TRACK_CreateImage(-1);
    }
    void Device::Destroy(VkImageView imageView) const {
        assert(imageView != VK_NULL_HANDLE);
        vkDestroyImageView(logical, imageView, SGF::g_VulkanAllocator);
        TRACK_IMAGE_VIEW(-1);
    }
    void Device::Destroy(VkFramebuffer framebuffer) const {
        assert(framebuffer != VK_NULL_HANDLE);
        vkDestroyFramebuffer(logical, framebuffer, SGF::g_VulkanAllocator);
        TRACK_FRAMEBUFFER(-1);
    }
    void Device::Destroy(VkRenderPass renderPass) const {
        assert(renderPass != VK_NULL_HANDLE);
        vkDestroyRenderPass(logical, renderPass, SGF::g_VulkanAllocator);
        TRACK_RENDER_PASS(-1);
    }
    void Device::Destroy(VkPipeline pipeline) const {
        assert(pipeline != VK_NULL_HANDLE);
        vkDestroyPipeline(logical, pipeline, SGF::g_VulkanAllocator);
        TRACK_PIPELINE(-1);
    }
    void Device::Destroy(VkPipelineLayout pipelineLayout) const {
        assert(pipelineLayout != VK_NULL_HANDLE);
        vkDestroyPipelineLayout(logical, pipelineLayout, SGF::g_VulkanAllocator);
        TRACK_PIPELINE_LAYOUT(-1);
    }
    void Device::Destroy(VkDescriptorSetLayout descriptorSetLayout) const {
        assert(descriptorSetLayout != VK_NULL_HANDLE);
        vkDestroyDescriptorSetLayout(logical, descriptorSetLayout, SGF::g_VulkanAllocator);
    ...
    void Device::Destroy(VkDescriptorPool descriptorPool) const {
        ...
        vkDestroyDescriptorPool(logical, descriptorPool, SGF::g_VulkanAllocator);
        TRACK_DESCRIPTOR_POOL(-1);
    }
    ...
    void Device::Destroy(VkCommandPool commandPool) const {
        ...
        vkDestroyCommandPool(logical, commandPool, SGF::g_VulkanAllocator);
        TRACK_COMMAND_POOL(-1);
    }
    void Device::Destroy(VkSampler sampler) const {
        assert(sampler != VK_NULL_HANDLE);
        vkDestroySampler(logical, sampler, SGF::g_VulkanAllocator);
        TRACK_SAMPLER(-1);
    }
    void Device::Destroy(VkSwapchainKHR swapchain) const {
        assert(swapchain != VK_NULL_HANDLE);
        vkDestroySwapchainKHR(logical, swapchain, SGF::g_VulkanAllocator);
        TRACK_SWAPCHAIN(-1);
    }
    void Device::Destroy(VkShaderModule module) const {
        assert(module != VK_NULL_HANDLE);
        vkDestroyShaderModule(logical, module, SGF::g_VulkanAllocator);
        TRACK_SHADER_MODULE(-1);
    }
