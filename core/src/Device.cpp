...
void SGF_Device::getQueueCreateInfos(VkPhysicalDevice device, VkSurfaceKHR surface, uint32_t* pIndexCount, VkDeviceQueueCreateInfo* pQueueCreateInfos, float* pQueuePriorityBuffer) {
    assert(device != VK_NULL_HANDLE);
    uint32_t count = 0;
	vkGetPhysicalDeviceQueueFamilyProperties(device, &count, nullptr);
	std::vector<VkQueueFamilyProperties> properties(count);
...
void SGF_Device::pickPhysicalDevice(uint32_t extensionCount, const char* const* pExtensions, const VkPhysicalDeviceFeatures* requiredFeatures,
...
void SGF_Device::createLogicalDevice(uint32_t extensionCount, const char* const* pExtensions, const VkPhysicalDeviceFeatures* requiredFeatures,
        const VkPhysicalDeviceFeatures* optionalFeatures, const VkPhysicalDeviceLimits* minLimits, VkSurfaceKHR surface) {
...
SGF_Device::SGF_Device(uint32_t deviceIndex, uint32_t extensionCount, const char* const* pExtensions, const VkPhysicalDeviceFeatures* requiredFeatures,
        const VkPhysicalDeviceFeatures* optionalFeatures, const VkPhysicalDeviceLimits* minLimits, SGF_Window* window, uint32_t g, uint32_t c, uint32_t t) 
...
SGF_Device::SGF_Device(uint32_t extensionCount, const char* const* pExtensions, const VkPhysicalDeviceFeatures* requiredFeatures,
...
SGF_Device::~SGF_Device() {
    SGF::debug("Device destructor called!");
    if (physical != VK_NULL_HANDLE) {
...
SGF_Device::SGF_Device(SGF_Device&& other) noexcept {
...
SGF_Device::Builder::Builder() : limits(defaultDeviceLimits) {}

constexpr VkPhysicalDeviceFeatures emptyFeatures = {};
SGF_Device SGF_Device::Builder::build() {
    return SGF_Device(deviceExtensionCount, deviceExtensions, &features, &optional, &limits,
...
uint32_t SGF_Device::findMemoryIndex(uint32_t typeBits, VkMemoryPropertyFlags flags) {
...
SGF_Memory SGF_Device::allocate(const VkMemoryAllocateInfo& info) {
    SGF_Memory mem;
...
SGF_Memory SGF_Device::allocate(const VkMemoryRequirements& req, VkMemoryPropertyFlags flags) {
...
SGF_Memory SGF_Device::allocate(VkBuffer buffer, VkMemoryPropertyFlags flags) {
    assert(buffer != VK_NULL_HANDLE);
    VkMemoryRequirements req;
    vkGetBufferMemoryRequirements(logical, buffer, &req);
    SGF_Memory mem = allocate(req, flags);
    if (vkBindBufferMemory(logical, buffer, mem, 0) != VK_SUCCESS) {
        SGF::fatal(SGF_ERROR_BIND_DEVICE_MEMORY);
    }
    return mem;
}
SGF_Memory SGF_Device::allocate(const VkBuffer* pBuffers, uint32_t bufferCount, VkMemoryPropertyFlags flags) {
...
SGF_Memory SGF_Device::allocate(VkImage image, VkMemoryPropertyFlags flags) {
...
SGF_Memory SGF_Device::allocate(const VkImage* pImages, uint32_t imageCount, VkMemoryPropertyFlags flags) {
    assert(imageCount != 0);
...
SGF_Memory SGF_Device::allocate(const VkBuffer* pBuffers, uint32_t bufferCount, const VkImage* pImages, uint32_t imageCount, VkMemoryPropertyFlags flags) {
    assert(imageCount != 0);
    assert(pImages != nullptr);
    assert(bufferCount != 0);
    assert(pBuffers != nullptr);
...
#pragma region IMAGE
SGF_Image SGF_Device::image(const VkImageCreateInfo& info) {
...
SGF_Image SGF_Device::image1D(uint32_t length, VkFormat format, VkImageUsageFlags usage, uint32_t mipLevelCount, VkSampleCountFlagBits samples, VkImageCreateFlags flags) {
...
SGF_Image SGF_Device::image2D(uint32_t width, uint32_t height, VkFormat format, VkImageUsageFlags usage, uint32_t mipLevelCount, VkSampleCountFlagBits samples, VkImageCreateFlags flags) {
    assert(graphicsCount != 0);
    VkImageCreateInfo info{};
    info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    info.arrayLayers = 1;
...
SGF_Image SGF_Device::image3D(uint32_t width, uint32_t height, uint32_t depth, VkFormat format, VkImageUsageFlags usage, uint32_t mipLevelCount, VkSampleCountFlagBits samples, VkImageCreateFlags flags) {
    assert(graphicsCount != 0);
    VkImageCreateInfo info{};
    info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    info.arrayLayers = 1;
...
SGF_Image SGF_Device::imageArray1D(uint32_t length, uint32_t arraySize, VkFormat format, VkImageUsageFlags usage, uint32_t mipLevelCount, VkSampleCountFlagBits samples, VkImageCreateFlags flags) {
    VkImageCreateInfo info{};
    info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
...
SGF_Image SGF_Device::imageArray2D(uint32_t width, uint32_t height, uint32_t arraySize, VkFormat format, VkImageUsageFlags usage, uint32_t mipLevelCount, VkSampleCountFlagBits samples, VkImageCreateFlags flags) {
    VkImageCreateInfo info{};
    info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    info.arrayLayers = arraySize;
    info.mipLevels = mipLevelCount;
...
SGF_Image SGF_Device::imageArray3D(uint32_t width, uint32_t height, uint32_t depth, uint32_t arraySize, VkFormat format, VkImageUsageFlags usage, uint32_t mipLevelCount, VkSampleCountFlagBits samples, VkImageCreateFlags flags) {
    VkImageCreateInfo info{};
    info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    info.arrayLayers = arraySize;
    info.mipLevels = mipLevelCount;
...
SGF_ImageView SGF_Device::imageView(const VkImageViewCreateInfo& info) {
    SGF_ImageView view;
...
SGF_ImageView SGF_Device::imageView1D(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer) {
...
SGF_ImageView SGF_Device::imageView2D(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer) {
...
SGF_ImageView SGF_Device::imageView3D(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer) {
...
SGF_ImageView SGF_Device::imageViewCube(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer) {
...
SGF_ImageView SGF_Device::imageArrayView1D(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer, uint32_t arraySize) {
...
SGF_ImageView SGF_Device::imageArrayView2D(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer, uint32_t arraySize) {
...
SGF_ImageView SGF_Device::imageArrayViewCube(VkImage image, VkFormat format, VkImageAspectFlags imageAspect, uint32_t mipLevel, uint32_t levelCount, uint32_t arrayLayer, uint32_t arraySize) {
...
VkFramebuffer SGF_Device::framebuffer(VkRenderPass renderPass, const VkImageView* pAttachments, uint32_t attachmentCount, uint32_t width, uint32_t height, uint32_t layerCount) {
...
VkRenderPass SGF_Device::renderPass(const VkAttachmentDescription* pAttachments, uint32_t attachmentCount, const VkSubpassDescription* pSubpasses, uint32_t subpassCount, const VkSubpassDependency* pDependencies, uint32_t dependenfyCount) {
...
VkFormat SGF_Device::getSupportedFormat(const VkFormat* pCandidates, uint32_t candidateCount, VkFormatFeatureFlags features, VkImageTiling tiling) {
