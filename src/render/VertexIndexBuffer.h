...
public:
	inline void allocate(uint32_t maxVertices, uint32_t maxIndices) {
		...
		vertexBuffer = vkl::createBuffer(Core.LogicalDevice, sizeof(SveModelVertex) * maxVertices, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, Core.GraphicsIndex);
		indexBuffer = vkl::createBuffer(Core.LogicalDevice, sizeof(uint32_t) * maxIndices, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, Core.GraphicsIndex);
		auto req = vkl::getBufferMemoryRequirements(Core.LogicalDevice, vertexBuffer);
		auto index_req = vkl::getBufferMemoryRequirements(Core.LogicalDevice, indexBuffer);
		...
		deviceMemory = vkl::allocateMemory(Core.LogicalDevice, Core.PhysicalDevice, req, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		vkBindBufferMemory(Core.LogicalDevice, vertexBuffer, deviceMemory, 0);
		vkBindBufferMemory(Core.LogicalDevice, indexBuffer, deviceMemory, indexOffset);
	}
	...
	inline void free() {
		...
		vkl::freeMemory(Core.LogicalDevice, deviceMemory);
